...
instance Monoid TxSkeleton where
  mempty = TxSkeleton{txSkelOuts = mempty, txSkelLocktime = Nothing}
...
buildTxFromSkeleton ::
  ...
  TxSkeleton ->
  Haskoin.Tx
buildTxFromSkeleton ctx selectIns TxSkeleton{..} =
  Haskoin.Tx 2 (toIn <$> selectIns) (toOut <$> txSkelOuts) [] (fromMaybe 0 txSkelLocktime)
 where
  toIn op =
    Haskoin.TxIn
      op
